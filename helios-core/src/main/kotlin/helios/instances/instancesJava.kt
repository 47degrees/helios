...
interface ListEncoderInstance<in A> : Encoder<List<A>> {
    ...
    companion object {
        operator fun <A> invoke(encoderA: Encoder<A>): Encoder<List<A>> = object : ListEncoderInstance<A> {
...
interface ListDecoderInstance<A> : Decoder<List<A>> {
    ...
    companion object {
        operator fun <A> invoke(decoderA: Decoder<A>): Decoder<List<A>> = object : ListDecoderInstance<A> {
