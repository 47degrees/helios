 ...
 * it.
 */
class ChannelParser<J>(private val ch: ReadableByteChannel, bufferSize: Int) : SyncParser<J>,
  ByteBasedParser<J> {

  companion object {
    private const val DefaultBufferSize = 1048576

    private const val ParseAsStringThreshold = 20 * 1048576
  ...
  private var bufSize: Int = computeBufferSize(bufferSize)
  private var mask: Int = bufSize - 1
  private var allSize: Int = bufSize * 2

  // these are the actual byte arrays we'll use
  private var curr = ByteArray(bufSize)
  private var next = ByteArray(bufSize)

  // these are the byte counts for each array
