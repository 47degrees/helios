...
fun Gen.Companion.short(): Gen<Short> = choose(Short.MIN_VALUE.toInt(), Short.MAX_VALUE.toInt()).map(Int::toShort)

fun Gen.Companion.byte(): Gen<Byte> = choose(Byte.MIN_VALUE.toInt(), Byte.MAX_VALUE.toInt()).map(Int::toByte)

fun Gen.Companion.bigDecimal(): Gen<BigDecimal> =
  Gen.double().filterNot { it.isInfinite() || it.isNaN() }.map(Double::toBigDecimal)
