...
fun <T> genJsArray(EN: Encoder<T>, valid: Gen<T>): Gen<JsArray> = Gen.create {
    Gen.list(valid).generate().map(EN::encode).let(::JsArray)
}

inline fun <reified T> genJsArray(valid: Gen<T>, EN: Encoder<T> = encoder()): Gen<JsArray> = Gen.create {
    Gen.list(valid).generate().map(EN::encode).let(::JsArray)
}
...
fun <T> genJson(EN: Encoder<T>, valid: Gen<T>): Gen<Json> = Gen.create {
    valid.generate().let(EN::encode)
}

inline fun <reified T> genJson(valid: Gen<T>, EN: Encoder<T> = encoder()): Gen<Json> = Gen.create {
    valid.generate().let(EN::encode)
}
