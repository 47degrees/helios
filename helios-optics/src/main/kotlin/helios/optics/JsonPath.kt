...
data class JsonPath(val json: Optional<Json, Json>) {
     ...
     */
    val boolean: Optional<Json, Boolean> = json compose Json.jsBoolean compose JsBoolean.iso

    /**
     * Extract value as [CharSequence] from path.
     */
    val charseq: Optional<Json, CharSequence> = json compose Json.jsString compose JsString.iso
     ...
     */
    val decimal: Optional<Json, String> = jsnumber compose JsNumber.jsDecimal compose JsDecimal.iso

    /**
     * Extract value as [Long] from path.
     */
    val long: Optional<Json, Long> = jsnumber compose JsNumber.jsLong compose JsLong.iso

    /**
     * Extract value as [Float] from path.
     */
    val float: Optional<Json, Float> = jsnumber compose JsNumber.jsFloat compose JsFloat.iso

    /**
     * Extract value as [Int] from path.
     */
    val int: Optional<Json, Int> = jsnumber compose JsNumber.jsInt compose JsInt.iso

    /**
     * Extract [JsArray] as `List<Json>` from path.
     */
    val array: Optional<Json, List<Json>> = json compose Json.jsArray compose JsArray.iso

    /**
     * Extract [JsObject] as `Map<String, Json>` from path.
     */
    val `object`: Optional<Json, Map<String, Json>> = json compose Json.jsObject compose JsObject.iso
     ...
     */
    fun at(field: String): Optional<Json, Option<Json>> = json compose Json.jsObject compose JsObject.at().at(field)
